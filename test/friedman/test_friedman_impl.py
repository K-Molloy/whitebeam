...
def test_friedman_depth(param_depth):

    X_train, X_test, y_train, y_test = generate_binary_classification()

    model = FriedmanTreeClassifier(max_depth=param_depth)
    model.fit(X_train, y_train)
    y_hat = model.predict(X_test)
...
def test_friedman_split_leaf(param_split, param_leaf):

    X_train, X_test, y_train, y_test = generate_binary_classification()
    model = FriedmanTreeClassifier(max_depth=4,
